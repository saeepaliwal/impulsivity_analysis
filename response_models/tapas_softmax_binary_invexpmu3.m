function logp = tapas_softmax_binary_invexpmu3(r, infStates, ptrans)
...
% Remap the decision temperature (be) to 1/mu3
be_vol = 1./exp(infStates(:,3,1));
...
for j = 1:size(r.y,2)
	...
	if size(r.u,2) == 1
		...
		logp(not(ismember(1:length(logp),r.irr)),j) = -log(1+exp(-be_vol.*(2.*x-1).*(2.*y-1)));
	end
	...
	if size(r.u,2) == 3
		% Calculate log-probabilities for non-irregular trials
		logp(not(ismember(1:length(logp),r.irr)),j) = -log(1+exp(-be_vol.*(r1.*x-r0.*(1-x)).*(2.*y-1)));
	end
end
logp = sum(logp,2);
