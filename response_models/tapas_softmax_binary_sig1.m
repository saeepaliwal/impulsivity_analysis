function logp = tapas_softmax_binary_sig1(r, infStates, ptrans)
...
for j = 1:size(r.y,2)
	...
	if size(r.u,2) == 1
		...
		logp(not(ismember(1:length(logp),r.irr)),j) = -log(1+exp(-be.*(2.*x-1).*(2.*y-1)));
	end
	...
	if size(r.u,2) == 3
		% Calculate log-probabilities for non-irregular trials
		logp(not(ismember(1:length(logp),r.irr)),j) = -log(1+exp(-be.*(r1.*x-r0.*(1-x)).*(2.*y-1)));
	end
end
logp = sum(logp,2);
